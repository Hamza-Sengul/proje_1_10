...
class AdminLoginView(LoginView):
    ...
    def form_valid(self, form):
        ...
        # 1) Yönetici (superuser) değilse hata ver
        if not user.is_superuser:
            form.add_error(None, "Bu giriş sayfası sadece yöneticiler içindir!")
            return self.form_invalid(form)
        
        # 2) Yönetici telefon numarasını al (AdminProfile’de tutuyorsanız)
        try:
            phone_number = user.adminprofile.phone_number
        except:
            form.add_error(None, "Yönetici telefon numarası tanımlı değil!")
            return self.form_invalid(form)
        
        if not phone_number:
            form.add_error(None, "Yönetici telefon numarası boş olamaz!")
            return self.form_invalid(form)

        # 3) SMS ile OTP kodunu gönder
        otp_code = send_otp_via_sms(phone_number)
        
        # 4) OTP kodunu session’a kaydet
        self.request.session['pending_admin_id'] = user.id
        self.request.session['admin_otp_code'] = str(otp_code)
        
        # 5) Henüz login yapma, doğrulama sayfasına yönlendir
        messages.success(self.request, "Telefonunuza doğrulama kodu gönderildi. Lütfen kodu giriniz.")
        return redirect('admin_otp_verify')  # a
...
from .models import User  # Custom user modelunuz
from django.contrib.auth import update_session_auth_hash

@login_required
def admin_profile(request):
    # Sadece superuser (yönetici) bu sayfaya erişebilsin
    if not request.user.is_superuser:
        return redirect('home')
    
    # View içerisinde inline form tanımlaması
    class InlineAdminProfileForm(forms.ModelForm):
        new_password = forms.CharField(
            widget=forms.PasswordInput, required=False, label="Yeni Şifre"
        )
        confirm_password = forms.CharField(
            widget=forms.PasswordInput, required=False, label="Yeni Şifre Tekrar"
        )
    ...
    if request.method == "POST":
        form = InlineAdminProfileForm(request.POST, instance=request.user)
        if form.is_valid():
            user = form.save(commit=False)
            ...
            user.save()
            # Şifre güncellendiyse, oturumun devam etmesi için session'ı güncelleyin.
            if new_password:
                update_session_auth_hash(request, user)
            return redirect('admin_panel')
    else:
        form = InlineAdminProfileForm(instance=request.user)
...
def add_customer_admin(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            customer = form.save(commit=False)
            customer.rep = request.user  # Admin tarafından eklenen müşteride admin'i temsilci olarak atayabilirsiniz
            customer.save()
            return redirect('list_customers_admin')
    else:
        form = CustomerForm()
    return render(request, "add_customer_admin.html", {"form": form})
...
def send_otp_via_sms(to_phone_number):
    otp_code = random.randint(100000, 999999)  # 6 haneli
    
    client = Client(settings.TWILIO_ACCOUNT_SID, settings.TWILIO_AUTH_TOKEN)
    message = client.messages.create(
        body=f"Giriş kodunuz: {otp_code}",
        from_=settings.TWILIO_SMS_FROM,       # Satın aldığınız Twilio numarası
        to=to_phone_number                    # +90..., +1..., vs. uluslararası format
    )
    return otp_code 
def admin_otp_verify(request):
    if request.method == 'POST':
        ...
        # session'daki kod
        session_code = request.session.get('admin_otp_code')
        # session'daki admin ID
        pending_admin_id = request.session.get('pending_admin_id')
        
        if input_code and session_code and (input_code == session_code):
            # Kod eşleşti, user'ı bul
            UserModel = get_user_model()
            try:
                user = UserModel.objects.get(id=pending_admin_id, is_superuser=True)
            except UserModel.DoesNotExist:
                messages.error(request, "Geçersiz kullanıcı veya yetki hatası.")
                return redirect('admin_login')
            
            # Login ve session'ı temizle
            login(request, user)
            del request.session['admin_otp_code']
            del request.session['pending_admin_id']
            
            messages.success(request, "OTP doğrulaması başarılı. Yönetici paneline yönlendiriliyorsunuz.")
            return redirect('admin_panel')
        else:
            messages.error(request, "Doğrulama kodu hatalı!")
            return redirect('admin_otp_verify')
    
    return render(request, 'admin_otp_verify.html')
...
#         # Session'da sakladığımız değerleri çekelim
#         session_code = request.session.get('admin_otp_code')
#         pending_admin_id = request.session.get('pending_admin_id')
        
#         if input_code and session_code and (input_code == session_code):
#             # Kod eşleşti, artık login yapabiliriz
#             UserModel = get_user_model()
#             try:
#                 user = UserModel.objects.get(id=pending_admin_id, is_superuser=True)
#             except UserModel.DoesNotExist:
...
#             # Session'daki kodu temizleyelim
#             del request.session['admin_otp_code']
#             del request.session['pending_admin_id']
